...
const TodosProgramas = () => {
  ...
  const handleSave = async () => {
    try {
      const url = `https://bored-cuff-links-foal.cyclic.app/programas/${selectedProgram.id}`;
      const updatedData = {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post('https://bored-cuff-links-foal.cyclic.app/programas', programDetails)
      .then(response => {
        setNotification({ type: 'success', message: 'Programa criado com sucesso!' });
        return axios.get(`https://bored-cuff-links-foal.cyclic.app/programas?instituicaoNome=${instituicaoNome}`);
      })
      .then(response => {
        setProgramas(response.data);
      })
